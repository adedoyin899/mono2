...
| SYS-01–04 (Notifications, Transaction History, Help, Terms) | Notifications backend real as of Phase 9 (`TalentDashboard.tsx`'s panel now binds to real per-user data in `live` mode); no dedicated transaction-history or help/support screens yet | Notifications: built. Transaction history / help / terms: not built → Phase 10 |
