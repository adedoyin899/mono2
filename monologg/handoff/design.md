...
| PWA-08 (Scheduling) | Embedded inside `TalentDashboard.tsx` ("Availability" tab) | Built; backend calendar-sync provider is real as of Phase 8 (`POST /availability/:id/sync-calendar`, `GET /calendar/busy-times`), but this screen isn't wired to it — deliberately, per Phase 8's own kickoff scope ("provider layer, not the rich availability UX" — that's Phase 13) |
...
| PWA-11 (Calendar/Checkout sheet) + PWA-12 (Payment) | `Checkout.tsx` | Backend landed in Phase 6 (`POST /bookings/:id/pay` → Paystack-first escrow, webhook-authoritative). `Checkout.tsx` itself is **not yet wired to it** — still a 2.5-second `setTimeout` and still says "FINCRA" three times, a known Phase 6 gap left open deliberately (frontend wiring wasn't in scope for that phase's kickoff) rather than fixed silently. Rewiring this screen is unscheduled — do it as its own reviewed pass, not folded into a later phase by default |
...
| SYS-01–04 (Notifications, Transaction History, Help, Terms) | All four real as of Phase 9/10. Notifications: `TalentDashboard.tsx`'s panel binds to real per-user data. Transaction History (`/transactions`): new screen, owner-scoped fee breakdown. Help & Support (`/support`): new screen, static FAQ + real ticket submit/list. Terms/Privacy (`/legal/terms`, `/legal/privacy`): new versioned pages, content explicitly flagged as a legal-review-pending draft | Built |
...
- **Payment/escrow backend is real (Phase 6)**: Paystack-first `PaymentProvider` (Stripe/Airwallex stubbed behind the same interface), a ledger-based escrow hold (`POST /bookings/:id/pay` → `initEscrow` → client pays on Paystack → `POST /webhooks/paystack` — the *only* authority that sets `ESCROW_LOCKED`, HMAC-SHA512-verified and idempotent via a DB-unique `(paymentId, type, eventId)` constraint) through to release (`PATCH /bookings/:id/approve`, base−11% to the talent) and a dispute/refund path. **`Checkout.tsx` itself is not yet wired to any of this** — still a 2.5-second `setTimeout`, still says "FINCRA" three times, a deliberately-left-open gap (see `log.md` Session 16) since Phase 6's kickoff scope was API-only.
- **KYC + AI style-tagging backend is real (Phase 7)**, and built as two fully independent systems (X3): `KycProvider` (`POST /creators/me/verify` → `PROCESSING` → poll `GET /creators/me/verify` → `VERIFIED`/`FAILED`, the only writer of `Creator.verification`) and `AiTaggingProvider` (media confirm → real `taggingStatus` job → `POST /creators/me/media/:id/confirm`/`GET .../media/:id`, the only writer of `Creator.styleTags`). `CreatorOnboarding.tsx`'s old scripted "Thespian AI Verified" animation is gone: `live` mode now drives the processing view from the real job state; `mock` mode keeps a local timed simulation (no network, same as every other mock-mode screen) with the corrected copy. Every other screen's "Thespian Verified"/"AI verification"/"AI-screened" copy was audited and fixed (`Settings.tsx`, `TalentDashboard.tsx`, `LandingPage.tsx`, `AuthFlow.tsx`, `ClientDashboard.tsx`) — see `log.md` Session 17. **Still open:** no screen anywhere actually collects the legal name/DOB/ID fields needed to call `POST /creators/me/verify` — a creator can't yet self-serve identity verification from the UI. Deliberately left open (new UI scope, not a wiring pass); flagged in `implementation-plan.md`.
...
4. **~~No real payment/escrow integration~~ Backend done (Phase 6).** Paystack-first `PaymentProvider`, webhook-authoritative ledger-hold escrow, idempotent + concurrency-safe release/refund. `Checkout.tsx` itself is unchanged — still the fake delay, still says FINCRA — a known, deliberately-left-open gap (frontend rewiring wasn't in Phase 6's kickoff scope; see `log.md` Session 16).
5. **~~No real AI verification~~ Done (Phase 7).** KYC and AI style-tagging are real, independent systems (X3) — see Section 3 above. Identity-verification input form is still missing from the UI (known, deliberately-left-open gap).
6. **~~No calendar sync~~ Provider layer done (Phase 8).** Real Google OAuth (encrypted refresh tokens), `pushAvailability`, `getBusyTimes` (freebusy), and `createMeet` (real Meet links, best-effort-wired into the escrow-lock webhook). The Calendar "sync" button and any Meet-link UI are still UI-only/absent — deliberately: Phase 8's kickoff scope was explicitly "the provider layer," with the rich availability UX + its UI deferred to Phase 13.
7. **~~No notifications backend~~ Done (Phase 9).** Real in-app persistence (`Notification` table, `GET /notifications` paginated + unread count, `POST /notifications/:id/read`) and async, retrying email/SMS (SendGrid/Twilio behind `NotifyProvider`, a BullMQ-on-Redis queue in prod / in-process retry in dev-test, `NotificationPreference` opt-out per channel). `TalentDashboard.tsx`'s panel is wired to real data in `live` mode (mock mode keeps its original two fixture entries, no network). `Settings.tsx`'s existing "Notifications" toggle UI is still local-only/unwired — a pre-existing gap (that whole screen has no backend wiring yet), not something this phase needed to close.
8. **~~No transaction history / help-support / terms screens~~ Done (Phase 10).** Three genuinely new screens: `/transactions` (owner-scoped, fee breakdown, filterable), `/support` (FAQ + ticket submit/list), `/legal/terms` + `/legal/privacy` (versioned, draft-flagged). Terms acceptance now recorded (version + timestamp) at registration.
