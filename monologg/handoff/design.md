...
**Last updated:** 2026-07-29 (Session 14: Phase 4 — real authentication)
...
**A real Fastify server and real authentication now exist** (Phases 3–4) — this is no longer a 100%-frontend prototype, though most *domain* data (talents, orders, projects, stats) still comes from `apps/web`'s mock fixtures until Phase 5 wires real endpoints:
- **Server exists, but only auth is real.** `apps/api/src` is a running Fastify app (`pnpm --filter @monologg/api run dev` → `http://localhost:3001`): CORS, Helmet, global + per-route rate limiting, structured pino logging, a sanitizing central error handler, `GET /api/v1/health` (real DB round-trip), and the full `/api/v1/auth/*` surface (below). Every other domain resource (creators, rate cards, bookings, order rooms, …) has **no route yet** — that's Phase 5.
- **Database is real and migrated, but `apps/web` doesn't read from it yet.** `apps/api/prisma/schema.prisma` (15 models, Phase 2) is migrated against the Supabase project provisioned in Phase 1, and `apps/api/prisma/seed.ts` reproduces the prototype's demo data there. Every list of data in the *running frontend* (orders, talents, projects, stats, messages) is still a hardcoded JavaScript constant defined at the top of its page file (e.g. `TALENTS`, `PROJECTS`, `ORDERS`, `STATS` in `ClientDashboard.tsx`/`TalentDashboard.tsx`) until Phase 5 flips `VITE_API_MODE=live` behind real endpoints.
...
1. **~~No API/backend service of any kind.~~ Done (Phases 1–3).** A running Fastify server exists with health/auth routes; domain resource routes (creators, bookings, etc.) land in Phase 5.
2. **No database wired into the running app.** The schema, migration, and seed exist in Supabase (Phase 2) but nothing in `apps/web` queries it yet — mock constants are still what every screen renders. → Phase 5 (endpoints + `VITE_API_MODE=live`).
...
- **Local dev API:** `pnpm --filter @monologg/api run dev` → `http://localhost:3001` (`GET /api/v1/health`, `/api/v1/auth/*`). Needs `apps/api/.env` (copy `.env.example`) — validated at boot by `src/config/env.ts`; missing/malformed vars fail fast with a specific message. Note: the two dev servers aren't proxied to each other yet, so `apps/web` with `VITE_API_MODE=live` won't actually reach `localhost:3001` for `/api/v1/...` calls without a Vite proxy — that's Phase 5+ territory once there's more than just auth to wire end-to-end locally.
