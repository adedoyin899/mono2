...
**Last updated:** 2026-07-28 (Session 12: Prisma schema, migration, and seed against Supabase)
...
- **No database wired into the running app yet — but the real schema now exists in Supabase.** `apps/api/prisma/schema.prisma` (15 models, Phase 2) is migrated against the Supabase project provisioned in Phase 1, and `apps/api/prisma/seed.ts` reproduces the prototype's demo data there (idempotent, one `Booking` per `BookingState`). Nothing in `apps/web` reads from it yet — every list of data (orders, talents, projects, stats, messages) is still a hardcoded JavaScript constant defined at the top of its page file (e.g. `TALENTS`, `PROJECTS`, `ORDERS`, `STATS` in `ClientDashboard.tsx`/`TalentDashboard.tsx`) until Phase 5 flips `VITE_API_MODE=live` behind real endpoints.
...
2. **No database wired into the running app.** The schema, migration, and seed exist in Supabase (Phase 2, Session 12) but nothing in `apps/web` queries it yet — mock constants are still what every screen renders. → Phase 3 (server) / Phase 5 (endpoints + `VITE_API_MODE=live`).
