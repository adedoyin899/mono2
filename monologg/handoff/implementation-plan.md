...
- [x] **Global Editorial Navigation & Footer**: Redesigned `WebsiteHeader.tsx` with floating glass editorial bar, NotionInter 500 links, 8px rounded controls, and Signal Blue CTA while maintaining full auth session sync. Updated `WebsiteFooter.tsx` with warm paper styling, 4 structured columns, and escrow trust badge.
- [x] **Landing Page Editorial Overhaul (`LandingPage.tsx`)**:
  - Hero with pill announcement badge, tight-tracking display headline, and 5-performer photo showcase strip.
  - "Find Performers for Every Kind of Work" 8-category exploration grid with **large 28px icons** in 56px rounded pastel squircles.
  - Replaced solid neon blocks with **airy transparent/pastel-like washes**, 20px rounded corners, subtle translucent borders, and **large 32px icons** in 64px squircles across all 6 core features.
  - 10-item FAQ accordion on 2-column warm paper card layout.
  - Midnight workspace final conversion banner and newsletter subscription bar.
- [x] **Product Suite Overhaul (`ProductPage.tsx`)**: Upgraded Performer Portal 6 feature cards with large icons & pastel squircle containers, transformed Client Portal into an airy transparent pastel lavender wash with large icons, and refined Thespian AI Engine dual cards.
- [x] **Transparent Pricing & Fee Simulator (`PricingPage.tsx`)**: Clean comparison tables with warm paper headers, subtle row borders, green checkmarks, performer avatar cluster, and partner brand logos (Vimeo, Netflix, Prime Video, Paystack, Spotify, Twitch).
- [x] **Resources & Investor Hub (`ResourcesPage.tsx`)**: Terms of Service & Support cards with large icons, email contact row with social icons, and complete multi-field "Become an Investor" application form with instant feedback state.
- [x] **Verification**: Clean typecheck (0 errors across monorepo), 100% web test pass (24/24 files, 97/97 tests), 100% API test pass (56/56 files, 580/580 tests), and Vite production build (2.30s).
