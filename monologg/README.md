...
**All 18 phases of `features.md` (0–17) are built** — Postgres/Prisma, real auth (JWT + argon2id), real Paystack-first escrow (fully wired into `Checkout.tsx` as of Phase 13), real KYC (Smile Identity) + AI style-tagging as two independent systems, real Google Calendar/Meet, real notifications, production hardening (security headers, coverage-gated tests, observability, Docker deploy), rich time-slot availability, two-sided project applications with an applicant cap, a public logged-out marketplace profile, and the flagship external-visitor deferred-account booking flow. `apps/web`'s `api-client` seam (`apps/web/src/lib/api-client.ts`) still defaults to `VITE_API_MODE=mock` (local fixtures) so the frontend can be developed/demoed with zero setup; flip it to `live` (or run `docker compose up --build` from this folder) to talk to the real API. **Phase 17 (an independent QA/security/UAT pass) found real, still-open gaps before production cutover** — a security fix needed in identity-verification review, no PWA install/offline infrastructure, UAT/NDPA sign-off both pending — see `monologg/qa/2026-07-31-phase17/README.md` for the current gate status. Full detail in `handoff/design.md`; the full phase-by-phase plan and what's actually done vs. open is `handoff/features.md` + `apps/api/README.md`.
